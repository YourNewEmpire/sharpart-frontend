...
export default async (req: NextApiRequest, res: NextApiResponse) => {
      ...
      setTimeout(async () => {
            ...
            if (price > newPrice) {
                  res.status(202).json({newPrice: newPrice, result: 'you lossed', address: user, })
            }
            else if(price === newPrice) {
                  res.status(202).json({newPrice: price, result: 'price held'})
            }
            else {
                  await transferTo().then(response => {
                        res.status(201).json({ newPrice: newPrice, result: 'you won', address: user, txHash: response })
                  })
            }
      }, 5000)
